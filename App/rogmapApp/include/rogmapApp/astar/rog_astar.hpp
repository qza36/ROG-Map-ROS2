#ifndef ROG_ASTAR_HPP
#define ROG_ASTAR_HPP
#include "rogmapApp/astar/types.hpp"
#include "rogmapApp/astar/config.hpp"
#include "rog_map/rog_map.h"
#include "memory"
#include "rogmapApp/utils/visualization.hpp"
...
namespace rog_astar {
    ...
    public:
        ...
        AStar(const rclcpp::Node::SharedPtr& node_ptr, 
              const rog_map::ROGMap::Ptr& map_ptr) : node_ptr_(node_ptr), cfg_(node_ptr) {
            map_ptr_ = map_ptr;
            vis_ptr_ = std::make_shared<visualization::Visualization>(node_ptr_);
        ...
        bool pathSearch(const Vec3f& start,
            ...
            if (!isStateValid(ps_.start)) {
                RCLCPP_ERROR(node_ptr_->get_logger(), "[A*\]: Start pos collide or out of bound");
                return false;
            }
            if (!isStateValid(ps_.goal)) {
                RCLCPP_ERROR(node_ptr_->get_logger(), "[A*\]: Goal pos collide or out of bound");
                return false;
            }
            ...
            RCLCPP_INFO(node_ptr_->get_logger(), "[RRT*\]: RRT starts planning a path");
            RCLCPP_INFO(node_ptr_->get_logger(), "	Start at: %f, %f, %f", ps_.start(0), ps_.start(1), ps_.start(2));
            RCLCPP_INFO(node_ptr_->get_logger(), "	Goal at: %f, %f, %f", ps_.goal(0), ps_.goal(1), ps_.goal(2));
    ...
    private:
        ...
        bool isStateValid(const Vec3f& pos) {
            ...
            return false; // Should not reach here if flags are set correctly
        }
        ...
        double getHeu(GridNodePtr node1, GridNodePtr node2, int type = DIAG) const {
            ...
            default: {
                RCLCPP_ERROR(node_ptr_->get_logger(), " -- [A*\] Wrong hue type");
                return 0;
            }
            }
        }
        ...
        bool astar_search() {
            ...
            rclcpp::Time time_1 = node_ptr_->get_clock()->now();
            ++rounds_;
            ...
            while (!open_set.empty()) {
                ...
                rclcpp::Time time_2 = node_ptr_->get_clock()->now();
                if (!cfg_.visualize_process_en && (time_2 - time_1).seconds() > ps_.max_time) {
                    RCLCPP_WARN(node_ptr_->get_logger(), "Failed in A star path searching !!! %lf seconds time limit exceeded.", ps_.max_time);
                    return false;
                }
            }
            rclcpp::Time time_2 = node_ptr_->get_clock()->now();
            if ((time_2 - time_1).seconds() > ps_.max_time) {
                RCLCPP_WARN(node_ptr_->get_logger(), "Time consume in A star path finding is %.3fs, iter=%d", (time_2 - time_1).seconds(),
                         num_iter);
                return false;
            }
